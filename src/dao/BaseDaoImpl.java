...
public abstract class BaseDaoImpl<T> implements BaseDao<T> {
	  ...
	  @Override
	    ...
	    public <K> List<K> findVO(String hql, int page, int rows) {
	        ...
	        if(page!=0)
	          ...
	          return q.setFirstResult(0).setMaxResults(rows).list();
	    }
	/**
	 * 提交数据并清空缓存
	 */
	...
	public void delete(final Serializable id) {
		...
		if (tableName != null && tableName != "") {
			SQLQuery sqlQuery = sessionFactory.getCurrentSession()
					.createSQLQuery("delete from " + tableName + " where id=?");
	...
	public <T> T findByProperty(final String propertyName, final Object value) {
		if (propertyName != null && value != null) {
			...
			Query query = sessionFactory.getCurrentSession().createQuery(stringBuffer.toString()).setParameter("value",
					value);
	...
	public <T> T findByProperties(final String[] propertyNames, final Object[] values) {
		if (propertyNames.length != 0 && values.length != 0 && propertyNames.length == values.length) {
			...
			Query query = DaoUtil.createConditionQuery(sessionFactory, stringBuffer, propertyNames, values, "id",
					false);
			// 执行查询
			return (T) query.uniqueResult();
		}
	...
	public <T> List<T> findAllByPage(final String propertyName, final boolean desc, final Integer startRow,
			final Integer pageSize) {
		String o = desc ? "desc" : "asc";
		return sessionFactory.getCurrentSession()
				.createQuery("from " + clazz.getSimpleName() + " obj order by obj." + propertyName + " " + o)
				.setFirstResult(startRow).setMaxResults(pageSize).list();
	}
	...
	public Long findAllCount() {
		return Long.valueOf(String.valueOf(sessionFactory.getCurrentSession().createSQLQuery("select count(0) from " + DaoUtil.getTableName(clazz)).uniqueResult()));			
	}
	...
	/**
	 * 用HQL Srcoll查询 大量查询时获取较高性能,此查询依赖于游标，返回的数据为查询的行值数组（Object[]） 可用于多表联合查询
	 * 
	 * @param HQL
	 *            HQL查询语句
	 * @param startRow
	...
	public ScrollableResults batchGetDataScrollByHQL(final String HQL, final Integer startRow, final Integer pageSize) {
		Query query = sessionFactory.getCurrentSession().createQuery(HQL).setFirstResult(startRow)
	...
	public ScrollableResults batchGetDataScrollBySQL(final String SQL, final Integer startRow, final Integer pageSize) {
		Query query = sessionFactory.getCurrentSession().createSQLQuery(SQL).setFirstResult(startRow)
				.setMaxResults(pageSize);
	 ...
	 */
	@Override
	public <T> List<T> findByProperty(final String[] propertyNames, final Object[] values, final Integer startRow,
			final Integer pageSize, final String sortColumn, final boolean isAsc) {
		if (propertyNames.length != 0 && values.length != 0 && propertyNames.length == values.length) {
			String clazzName = clazz.getSimpleName();
			StringBuffer stringBuffer = new StringBuffer("select Object(obj) from ");
			stringBuffer.append(clazzName);
			Query query = DaoUtil.createConditionQuery(sessionFactory, stringBuffer, propertyNames, values, sortColumn,
					isAsc);
			// 分页
			query.setFirstResult(startRow);
			query.setMaxResults(pageSize);
	 ...
	 */
	@Override
	public <T> List<T> findByProperty(final String propertyName, final Object value, final Integer startRow,
			final Integer pageSize, final String sortColumn, final boolean isAsc) {
		String clazzName = clazz.getSimpleName();
		StringBuffer stringBuffer = new StringBuffer("select obj from ");
		stringBuffer.append(clazzName);
		Query query = DaoUtil.createSingleConditionHqlQuery(sessionFactory, stringBuffer, propertyName, value,
				sortColumn, isAsc);
		// 分页
		query.setFirstResult(startRow);
		query.setMaxResults(pageSize);
	...
	public Long findByPropertyCount(final String propertyName, final Object value) {
		...
		StringBuffer stringBuffer = new StringBuffer("select count(0) from ");
		stringBuffer.append(clazzName);
		Query query = DaoUtil.createSingleConditionHqlQuery(sessionFactory, stringBuffer, propertyName, value, "id",
				true);
		return (Long) query.uniqueResult();
	}
	 ...
	 */
	@Override
	public <T> List<T> findByProperty(final T object, final Integer startRow, final Integer pageSize,
	 ...
	 */
	@Override
	public <T> List<T> findByProperty(final Map<String, Object> map, final Integer startRow, final Integer pageSize,
			final String sortColumn, final boolean isAsc) {
		List<Object[]> mapList = DaoUtil.changeMapToArray(map);
		String[] propertyNames = (String[]) mapList.get(0);
		Object[] values = mapList.get(1);
	 ...
	 * 
	 * @param sql
	 *            插入语句，如：insert into user_info(account,pwd) values
	 * @param values
	 *            需插入数据库值
	 */
	@Override
	public void batchInsertBySQL(final String sql, final List<Object[]> values) {
		...
		if (length != 0) {
			try {
				...
				e.printStackTrace();
				logger.info(
						"##com.blog.dao.SupperBaseDAOImp.batchInsertBySQL(final String sql, final List<Object[]> values)方法出错##");
	 ...
	 * +------+--------+ | code | value | +------+--------+ | 001 | 四川 | | 002 |
	 * 北京 | | 001 | 四川 | +------+--------+ eg:select new Map(d.code, d.value)
	 * from Dictionary d result:[{0=001，1=四川},{0=002，1=北京},{0=001，1=四川}] the
	 * result is List<Map>,one Map is one record,if not define the alias
	 * name,the map's key will be the column index and start with 0; if you
	 * define a alias name ,it to be like this: select new Map(d.value as value,
	 * d.code as code) from Dictionary d [{value=四川, code=001},{value=北京,
	 * code=002},{value=四川, code=001}] select new Map(d.value, d.code as code)
	 * from Dictionary d [{0=四川, code=001},{0=北京, code=002},{0=四川, code=001}]
	 * the map's key will be the alias name!
	 * 
	 * @param HQL
	 *            HQL语句，如：select new Map(d.code as code, d.value as value) from
	 *            Dictionary d;标明别名的‘as’是必须的
	 * @param startRow
	 *            开始行数
	 * @param pageSize
	 *            返回记录数
	...
	/**
	 * table data: +----+------+--------+ | id | code | value |
	 * +----+------+--------+ | 1 | 001 | 四川 | | 2 | 002 | 北京 | | 3 | 001 | 四川 |
	 * +----+------+--------+ eg:select new List(d.code,d.value) from Dictionary
	 * d result:[[001, 四川], [002, 北京], [001, 四川]] the result is List<List>, one
	 * inner list is filled by column's value using one record
	 * 
	 * @param HQL
	 *            HQL语句
	 * @param startRow
	 ...
	 * @return List
	 */
	public <T> List<T> getListBySQL(final String SQL, final Class<T> claz, final Integer startRow,
			final Integer pageSize) {
		Query query = sessionFactory.getCurrentSession().createSQLQuery(SQL)
	...
	@Override
	public List findBySqlWithJoin(final String SQL, final List<List> addClazz, final Integer startRow,
			final Integer pageSize) {
	...
	/**
	 * 查询多个实体，必须包含全字段 createSQLQuery("select {ar.*},{ca.*} from article_category
	 * ca,article_content ar where
	 * ar.id=ca.id").addEntity("ca",ArticleCategory.class).addEntity("ar",ArticleContents.class).list();
	 ...
	 * 
	 * @param SQL
	 *            SQL语句，可包含分页信息，如果不要分页pageSize要设为<=0
	 * @param alias
	 *            类名参数数组
	 * @param startRow
	 *            记录开始行数
	 ...
	 */
	@Override
	public List findMoreBeanBySql(final String SQL, final List<List> alias, final Integer startRow,
			final Integer pageSize) {
	...
	public <T> List<T> findObjsByHQL(final String HQL, final Integer startRow, final Integer pageSize) {
		return sessionFactory.getCurrentSession().createQuery(HQL).setFirstResult(startRow).setMaxResults(pageSize)
				.list();
	}
	...
	public <T> List<T> findEntityBySQL(String sql, Class<T> clas, Integer startRow, Integer pageSize) {
		return sessionFactory.getCurrentSession().createSQLQuery(sql).addEntity(clas).setFirstResult(startRow)
